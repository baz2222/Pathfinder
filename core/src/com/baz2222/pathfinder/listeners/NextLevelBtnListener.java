...
import com.baz2222.pathfinder.screen.LevelScreen;
import com.baz2222.pathfinder.tools.GameScreen;

public class NextLevelBtnListener extends InputListener {
    private Pathfinder game;
    public NextLevelBtnListener(Pathfinder game) {
        this.game = game;
    }

    @Override
    public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
        Pathfinder.log("next level button pressed");
        game.screenManager.levelOverScreen.onClose();
        game.screenManager.levelScreen = new LevelScreen(game);
        game.characterManager.enemies = new Array<>();
