package com.baz2222.pathfinder.character;
...
import com.baz2222.pathfinder.manager.CharacterManager;
...
public class Enemy2 extends Actor {
    ...
    public Enemy2(Pathfinder game, String name, float enemyX, float enemyY, boolean runRight) {
        this.game = game;
        setName(name);
        stepTimer = 0;
        trailTimer = 0;
        timer = 0;
        filter = new Filter();
        texture = game.characterManager.enemy2Atlas.findRegion("senemy").getTexture();
        ...
        frames.clear();
        //jump animation
        frames.add(new TextureRegion(texture, 14 * 64, 0, 64, 96));
        animations.put("enemyJump", new Animation<>(0.1f, frames));
        frames.clear();
        //fall animation
        frames.add(new TextureRegion(texture, 13 * 64, 0, 64, 96));
        animations.put("enemyFall", new Animation<>(0.1f, frames));
        frames.clear();
        //idle animation
        for (int i = 0; i < 13; i++) {
            frames.add(new TextureRegion(texture, i * 64, 64, 64, 96));
        }//for
        animations.put("enemyIdle", new Animation<>(0.1f, frames));
        frames.clear();
        //die animation
        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(texture, i * 64, 192, 64, 96));
        }//for
        ...
        shape.setAsBox(14 / game.scale, 30 / game.scale);
        ...
        fixtureDef.filter.categoryBits = game.box2DManager.enemy2Bit;
    ...
    public void draw(Batch batch, float parentAlpha) {
        if(game.gamePaused == false) {
            if (fixture.getFilterData().categoryBits == game.box2DManager.removedBit && body.isActive()) {
                ...
                setPosition((body.getPosition().x - getWidth() / 2) * game.scale, (body.getPosition().y - getHeight() * 0.35f) * game.scale);
                game.characterManager.onEnemy2Warp(this);
                batch.draw(getFrame(), getX(), getY());
            }//if not dead
        }//if not paused
    }//draw
